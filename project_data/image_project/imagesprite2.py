...
class ImageSprite2(Sprite):
    """이미지 스프라이트 클래스"""
    def __init__(self, x, y, image_path="/home/inteee/opencvZoo2/data/lenna.bmp", size=(100, 100)):
        super().__init__(x, y)
    ...
    def check_mouse_position(self, mouse_x, mouse_y):
        #topleft, topright, bottomleft에 있는지 확인 += 10 픽셀 범위
        if(self.image_points[0][0] - 10 <= mouse_x <= self.x + self.image_points[0][0] + 10 and self.image_points[0][1] - 10 <= mouse_y <= self.image_points[0][1] + 10):
            self.point_index = 0
            return True
         # topright
    ...
    def warpPerspective(self, x, y):
        ...
        self.image = cv2.warpAffine(self.image, self.size)
